...
public class SessionManager {
		...
		if (youAreTheDirector) {
			...
			if ( rol.equals("ApprenantCandidat") ){
				for( SerAgent sA:SelectSession.getCandidatsAp() ){
					CliAgent cA = new CliAgent(sA.getMatricule(), sA.getNom(), sA.getPrenom());
	...
	@GET
	@Path("/directeur/")
	public List<CliStage> ListeStageDirecteur(@PathParam("cleHashage") String cle, @QueryParam("role") String rol) {

		List<CliStage> ListeStage = new ArrayList<CliStage>();

		if ( rol.equals("directeur") ){
			for( SerStage s:donnees.lStage ){
				if( s.getDirecteurSer().getCleHashage() ==  cle ){
					CliStage nouveau=new CliStage();
					List<CliSession> ListeSessTmp = new ArrayList<CliSession>();
					...
					CliAgent clientDir = new CliAgent(s.getCliDirecteur().getMatricule(), s.getCliDirecteur().getNom(), s.getCliDirecteur().getPrenom());
