...
public class SessionManager {
	...
	public Response valideCandidats( @PathParam("cleHashage") String cle, @PathParam("CodeSession") int codeS ) {
		...
		if (youAreTheDirector) {
			...
			if ( rechercheAgent(cle , SelectSession.getCandidatsAp()) != null ) {
				AgentApAValider = rechercheAgent(cle , SelectSession.getCandidatsAp());
				SelectSession.validerApprenant(AgentApAValider);
				return Response.status(Status.ACCEPTED).entity(AgentApAValider).build();
			
				} else if ( rechercheAgent(cle , SelectSession.getCandidatsFo()) != null ){
					AgentApAValider = rechercheAgent(cle , SelectSession.getCandidatsFo());
	...
	public List<Client.CliAgent> ListeAgentSession(@PathParam("codeSession") int codeS, @QueryParam("role") String rol){
		List<Client.CliAgent> ListeCandidat=new ArrayList<Client.CliAgent>();
		SerSession SelectSession = rechercheSession(codeS,donnees.lSession);
		boolean youAreTheDirector = false;
